...
parser.add_argument(
    "--max_samples", type=str, default=256, help="EIF parameter: max_samples"
)
parser.add_argument(
    "--contamination",
    type=float,
...
parser.add_argument(
    "--model", type=str, default="EIF", help="Model to use: IF, EIF, EIF+"
)
parser.add_argument(
    "--interpretation",
    type=str,
...
parser.add_argument(
    "--compute_perf", action="store_true", help="If set compute the model performances"
)
parser.add_argument(
    "--n_quantiles",
    type=int,
...
assert interpretation in [
    ...
    "DIFFI",
    "KernelSHAP",
], "Interpretation method not recognized"
...
try:
    ...
    dict_time["fit"].setdefault(I.name, {}).setdefault(dataset.name, []).append(
        fit_time
    )
...
try:
    ...
    print("Model not recognized: creating a new key in the dict_time for the new model")
    dict_time["predict"].setdefault(I.name, {}).setdefault(dataset.name, []).append(
        predict_time
    )
...
if compute_GFI:
    if interpretation == "KernelSHAP":
        ...
        try:
            dict_time["importances"][interpretation][dataset.name].setdefault(
                f"background_{int(background*100)}", []
            ).append(importances_time)
        except:
            print(
                "Model not recognized: creating a new key in the dict_time for the new model"
            )
            dict_time["importances"].setdefault(interpretation, {}).setdefault(
                dataset.name, {}
            ).setdefault(f"background_{int(background*100)}", []).append(
                importances_time
            )
    else:
...
if compute_perf:
    ...
    print("#" * 50)
    performance_metrics, path = performance(
        y_pred=y_pred,
